...
export default function LessonPage() {
  ...
  // 2. Hook Complete Lesson
  const { completeLesson } = useCompleteLesson();

  // 3. Logic Navigation & Progress
  const nav = useMemo(() => {
  ...
  // Tính % hoàn thành dựa trên số lượng bài đã isCompleted
  ...
  const handleVideoEnded = async () => {
    if (courseId && lessonId) {
      // Chỉ gọi API nếu bài này chưa hoàn thành (tránh spam API)
          ...
          <div className="p-3">
            {lessons.map((item: any, idx: number) => (
              <LessonItem
                ...
                courseId={courseId}
                isCompleted={item.isCompleted} // Truyền xuống
              />
            ))}
          </div>
        </ScrollArea>
...
// --- SUB-COMPONENT: LESSON ITEM (CẬP NHẬT UI COMPLETE) ---
...
// ... Các component khác (ResourceCard, Skeleton, Utils...) GIỮ NGUYÊN
