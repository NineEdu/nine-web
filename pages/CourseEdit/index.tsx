...
import useReorderLessons from "@/hooks/lessons/useReorderLessons"; // Hook mới tạo
...
// --- SUB-COMPONENT: LESSON ITEM (MODIFIED FOR DRAGGING) ---
// Thêm props: provided, isDragging
    ...
    >
      <AccordionTrigger className="hover:no-underline py-4">
        <div className="flex items-center gap-3 w-full pr-4">
          {/* DRAG HANDLE - Chỉ cho phép kéo khi nắm vào icon này */}
          <div
            {...provided.dragHandleProps}
            className="cursor-grab hover:text-slate-600 active:cursor-grabbing p-1"
            onClick={(e) => e.stopPropagation()} // Quan trọng: Ngăn accordion mở khi kéo
          >
...
export default function CourseCurriculumPage() {
  ...
  // Local state để hiển thị Drag mượt mà
  const [items, setItems] = useState<any[]>([]);

  // Hook reorder API
  const { reorderLessons, isPending: isReordering } = useReorderLessons({
    onSuccess: () => {
      // Invalidate query để đảm bảo đồng bộ
      queryClient.invalidateQueries({ queryKey: ["lessons", courseId] });
    },
  });

  // Đồng bộ data từ API vào state local
  useEffect(() => {
    setItems(lessons);
  }, [lessons]);

  // Fix hydration cho DND
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Nếu vị trí không đổi thì return
    ...
    // 2. Chuẩn bị data chuẩn gửi backend
